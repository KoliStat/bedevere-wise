...
- [Enhanced] **Column stats reflect the filtered view.** Filtering a column now moves the side-panel histograms, counts, and value-frequency lists with the visible rows. Previously stats were always computed against the unfiltered source — confusing when half the data was hidden.
