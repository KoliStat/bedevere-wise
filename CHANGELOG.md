...
- [Feature] **Editor multi-cursor + find.** `Alt+ArrowUp` / `Alt+ArrowDown` drop a cursor above / below the current line. `Ctrl+F` opens the find panel (themed to match the Tokyonight palette — was previously default-white CodeMirror); F3 / Shift+F3 step through matches.
